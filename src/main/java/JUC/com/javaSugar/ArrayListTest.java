 ...
 *     <p>
 *     <p>ays.asList("1", "123");执行add 时报错 ,且foreach 进行删除时也会报错 要先进行Iterator
 *     或者将list转化成strem的形式进行lambda格式过滤
 */
public class ArrayListTest {
  public static void main(String[] args) {
    List<String> list1 = Arrays.asList("sfsf");
    boolean s1 = list1.stream().allMatch(s -> s.startsWith("s"));
    System.out.println(s1 + "\t 第19行");
